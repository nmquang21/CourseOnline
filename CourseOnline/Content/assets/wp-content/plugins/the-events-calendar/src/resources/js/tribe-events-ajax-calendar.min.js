    ...
    a(t).ready(function() {
        ...
        function u(e) {
            ...
            var r = e.parents(".tribe-events-calendar"),
                ...
                l = '[data-day="' + a.date + '"]',
                o = n.filter(l);
            a.has_events = e.hasClass("tribe-events-has-events"), s.removeClass("mobile-active").filter(l).addClass("mobile-active"), n.hide(), o.length ? o.show() : (i.append(tribe_tmpl("tribe_tmpl_month_mobile_day_header", a)), _(a.date))
        }
        ...
        function p() {
            ...
            if (a(t.getElementById("tribe-mobile-container")).length || a('<div id="tribe-mobile-container" />').insertAfter(i), e.length && e.is(".tribe-events-thismonth")) u(e);
            else {
        ...
        function b() {
            y.find(".tribe-events-calendar th").each(function() {
                var e = a(this),
                    ...
                    r = e.attr("title");
                g.is(".tribe-mobile") ? e.text(t) : e.text(r)
            })
        }

        function c(e) {
            g.is(".tribe-mobile") ? (b(), p()) : e && b()
        }
        ...
        function v() {
            n.invalid_date(s.date) || (a(".tribe-events-calendar").tribe_spin(), s.pushcount = 0, s.ajax_running = !0, s.popping || (s.params = {
                ...
                if (s.initial_load = !1, n.enable_inputs("#tribe_events_filters_form", "input, select"), e.success) {
                    ...
                    }, d && 0 === e.html && debug.warn("Month view ajax had an error in the query and returned 0."), r.cur_url = n.get_base_url();
                    var l = "";
                    l = a.isFunction(a.fn.parseHTML) ? a.parseHTML(e.html) : e.html, a("#tribe-events-content").replaceWith(l), c(!0), s.page_title = a("#tribe-events-header").data("title"), t.title = s.page_title, (a("#tribe-events.tribe-events-shortcode").length || s.do_string) && (-1 !== r.cur_url.indexOf("?") && (r.cur_url = r.cur_url.split("?")[0]), r.cur_url = r.cur_url + "?" + s.url_params), s.do_string && history.pushState({
                        tribe_date: s.date,
                        tribe_params: s.params
                    }, s.page_title, r.cur_url), s.pushstate && history.pushState({
                        tribe_date: s.date,
                        tribe_params: s.params
                    }, s.page_title, r.cur_url), a(i).trigger("tribe_ev_ajaxSuccess").trigger("tribe_ev_monthView_ajaxSuccess"), a(i).trigger("ajax-success.tribe").trigger("tribe_ev_monthView_ajaxSuccess"), d && debug.timeEnd("Month View Ajax Timer")
                }
            })) : s.url_params.length ? e.location = r.cur_url + "?" + s.url_params : e.location = r.cur_url)
        }
        var g = a("body"),
            f = a('[class^="tribe-events-nav-"] a'),
        ...
        if ("0" !== s.datepicker_format) {
            ...
            w = r.datepicker_formats.month[D], h && (h.length <= 7 && (h += "-01"), x.val(tribeDateFormat(h, E)))
        }
        if (r.datepicker_opts = {
                ...
                if (j = !0, s.date = i + "-" + o, l.no_bar() || l.live_ajax() && l.pushstate) {
                    if (s.ajax_running || s.updating_picker) return;
                    s.filter_cats ? r.cur_url = a(t.getElementById("tribe-events-header")).data("baseurl") + s.date + "/" : r.default_permalinks ? r.cur_url = k : r.cur_url = k + s.date + "/", s.popping = !1, n.pre_ajax(function() {
                        v()
                    })
                }
            ...
            var I = "action=tribe_calendar&eventDate=" + a("#tribe-events-header").data("date");
            r.params.length && (I = I + "&" + r.params), s.category && (I = I + "&tribe_event_category=" + s.category), n.is_featured() && (I += "&featured=1"), history.replaceState({
                tribe_params: I
        ...
        a(t.getElementById("tribe-events")).on("click", ".tribe-events-nav-previous, .tribe-events-nav-next", function(e) {
            if (e.preventDefault(), !s.ajax_running) {
                var t, i = a(this).find("a");
                s.date = i.data("month"), s.mdate = s.date + "-01", "0" !== s.datepicker_format ? n.update_picker(tribeDateFormat(s.mdate, E)) : n.update_picker(s.date), t = s.filter_cats ? a("#tribe-events-header").data("baseurl") : i.attr("href"), r.default_permalinks && (t = r.cur_url.split("?")[0]), n.update_base_url(t), s.popping = !1, n.pre_ajax(function() {
                    v()
                })
            }
            ...
            if (g.is(".tribe-mobile")) {
                e.preventDefault();
                var t = a(this).closest(".mobile-trigger");
                u(t)
            }
        }).on("click", ".mobile-trigger", function(e) {
            g.is(".tribe-mobile") && (e.preventDefault(), e.stopPropagation(), u(a(this)))
        ...
        }), d && debug.info("TEC Debug: tribe-events-ajax-calendar.js successfully loaded, Tribe Events Init finished"), d && debug.timeEnd("Tribe JS Init Timer")
    })
